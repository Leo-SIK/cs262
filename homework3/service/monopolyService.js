...
router.get("/players/:id/scores", playerScoreTime);
...
function playerScoreTime(req, res, next) {
    db.many(`SELECT name, score FROM Player, PlayerGame WHERE Player.ID = playerID AND Player.id=${req.params.id}`)
      .then(data => {
        returnDataOr404(res, data);
      })
      .catch(err => {
